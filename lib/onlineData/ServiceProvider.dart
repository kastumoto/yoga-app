...
Future<ModelDetailExerciseList?> getExerciseList(
  ...
  final response =
      await http.post(Uri.parse(ConstantUrl.urlGetALLWorkoutExerciseOfCategory), body: data);
  print("rescode===${response.statusCode}--${response.body}");
...
Future<ModelDetailExerciseList?> getMyPlanExerciseList(
  ...
  final response =
  await http.post(Uri.parse( ConstantUrl.urlGetMyPlanExercise,), body: data);
...
Future<ModelAddPurchasePlan?> addPurchasePlan(
    String planId, String purchaseDate, {String purchaseId="noId"}) async {
  String deviceId = await ConstantUrl.getDeviceId();
  String s = await PrefData.getUserDetail();
  ...
  if (s.isNotEmpty) {
    ...
    print("response--addpurchaseplan---${response.statusCode}===${userDetail.userId}====${session}==${deviceId}==${planId}===${purchaseDate}");
...
Future<Checkpurchaseplanday?> checkPurchasePlanDay() async {
  ...
  if (s.isNotEmpty) {
    ...
    String session = await PrefData.getSession();
    print(
        "userDetail purchase----$session--$deviceId==-$s--");
    ...
    if (checkValidResCode(response.statusCode) && response.body.isNotEmpty) {
      ...
      ModelCheckPurchasePlanDay _checkPurchasePlanDay =  ModelCheckPurchasePlanDay.fromJson(jsonDecode(response.body));
      print("response--checkpurchaseplanday1212---${response.body}");


      if (
          _checkPurchasePlanDay.data!.success == 1) {
...
//================ New Workout Module=============
Future<ModeBodySpecificCategory?> getAllBodySpecificCategory(BuildContext context) async {
  Map data = await ConstantUrl.getCommonParams();
...
Future<ModelOtherWorkoutCategoryList?> getAllOtherWorkoutCategoies(BuildContext context) async {
  Map data = await ConstantUrl.getCommonParams();

  final response =
  await http.post(Uri.parse(ConstantUrl.urlGetAllOtherWorkOutCategories), body: data);
  var value = ModelOtherWorkoutCategoryList.fromJson(jsonDecode(response.body));
  return value;
}

Future<ModelBodySpecificSubCategoryWorkouts?> getAllBodySpecificSubCategoryDetail(BuildContext context, String subCategoryId) async {
  Map data = await ConstantUrl.getCommonParams();
  data[ConstantUrl.varCatId] = subCategoryId;
  final response =
  await http.post(Uri.parse(ConstantUrl.urlGetAllWorkoutsofOneBodySpecific), body: data);
  var value = ModelBodySpecificSubCategoryWorkouts.fromJson(jsonDecode(response.body));
  return value;
}
