...
class ConstantWidget {
      ...
      builder: (context, setState) {
        return Focus(
          ...
          child: Container(
              ...
              margin: EdgeInsets.symmetric(
                  vertical: getScreenPercentSize(context, 1.2),
                horizontal: isHorizontal==null? 0: getScreenPercentSize(context, 2),
              ),
              alignment: Alignment.centerLeft,
  ...
  static Widget getIconButtonWidget(
    ...
    return GestureDetector(
      child: Container(
        ...
        margin: EdgeInsets.symmetric(
            vertical: ConstantWidget.getScreenPercentSize(context, 1.2),
          horizontal: isHorizontal==null? 0 :  ConstantWidget.getScreenPercentSize(context, 2.5)
        ),
        decoration: getDefaultDecoration(
            radius: radius,
  ...
  static Widget getBorderWithBlackBackgroundButtonWidget(
      BuildContext context, String s, Function function,
      {Color? borderColor, Color? fillColor ,double? btnHeight, Color? fontColor}) {
    double height = btnHeight == null
        ? ConstantWidget.getDefaultButtonSize(context)
        : btnHeight;
    double radius = ConstantWidget.getPercentSize(height, 20);
  ...
  static Future<bool> isPlanValid() async {
    Checkpurchaseplanday? _checkPurchasePlanDay =
    await checkPurchasePlanDay();
    ...
    if (_checkPurchasePlanDay != null ) {
      int getVal =
          _checkPurchasePlanDay.expireDay ?? 0;

      String isActive =
          _checkPurchasePlanDay.isActive ?? "0";
  ...
  FutureBuilder<bool>(
          ...
          builder: (context, snapshot) {
            ...
            if (snapshot.data != null && !snapshot.data!) {
                ...
                if(1==0){//if(isPro.value==true){
                  return Container();
                }else{
                  return     Align(
                    alignment: alignment == null ? Alignment.topRight : alignment,
                    child: Container(
                      decoration: BoxDecoration(
                        color: Colors.white,
    ...
    builder: (context, snapshot) {
      ...
      if (snapshot.data != null && !snapshot.data!) {
        ...
        if(1==0){//if(isPro.value==true){
          ...
          return     Container(
                        margin: EdgeInsets.only(top: ConstantWidget.getPercentSize(
                        quickWorkoutHeight??10, 3.5), right: ConstantWidget.getPercentSize(
                        quickWorkoutHeight??10, 11)),
                        decoration: BoxDecoration(
                        border: Border.all(color: color!),
                        color: color,
                        borderRadius:
                        BorderRadius.circular(ConstantWidget.getPercentSize(
